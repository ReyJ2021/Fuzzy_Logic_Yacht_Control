...
%         V_in(4) x translation in n-frame  (reference to ground)
%         V_in(5) y translation in n-frame  (reference to ground) 
%         V_in(6) phi Euler angle  (rotation from n-frame to b-frame)
%         V_in(7) psi Euler angle  (rotation from n-frame to b-frame)
%         V_in(8) u  forward velocity in b-frame
%         V_in(9) v  side velocity in b-frame
%         V_in(10) p - roll (angular velocity) in b-frame
%         V_in(11) r - yaw (angular velocity) in b-frame
%         V_in(12) wind speed - (replace par.vt)
%         V_in(13) wind direction - (replace par.alpha_w)
%       ]
...
% the sail
v_t = [ V_in(11)*cos(V_in(11)); V_in(11)*sin(V_in(11)); 0 ];
...
% sail angle computation from max sail input
% (sail luffing or not)
